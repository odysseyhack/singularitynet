...
class ForecastServicer(grpc_bt_grpc.ForecastServicer):
    ...
    def forecast(self, request, context):
        ...
        try:
            ...
            if wait_for_gpu(gpu_queue_id):
                return self.response

            # Lock GPU usage
            acquire_gpu(gpu_queue_id)

            fc = Forecast(request.model_file, self.csv_file, request.input_dim)
            y_data, predictions = fc.forecast(request.num_points)
            self.response = Output(output_points=predictions)

            log.debug("forecast({},{}) = {}".format(request.model_file, request.num_points, self.response.output_points))
            # Unlock GPU usage
